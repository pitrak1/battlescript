...
public static class ArgumentTransferTests
{
    ...
    public class WithInstructionArguments
    {
        ...
        public void SupportsPositionalArguments()
        {
            ...
            Assertions.AssertVariablesEqual(memory.Scopes.First()["x"], BsTypes.Create(memory, "int", 5));
            Assertions.AssertVariablesEqual(memory.Scopes.First()["y"], BsTypes.Create(memory, "int", 6));
        }
        ...
        public void SupportsKeywordArguments()
        {
            ...
            Assertions.AssertVariablesEqual(memory.Scopes.First()["x"], BsTypes.Create(memory, "int", 6));
            Assertions.AssertVariablesEqual(memory.Scopes.First()["y"], BsTypes.Create(memory, "int", 5));
        }
        ...
        public void IgnoresDefaultValueIfPositionalArgumentGiven()
        {
            ...
            ArgumentTransfer.RunAndApply(memory, arguments, parameters);
            Assertions.AssertVariablesEqual(memory.Scopes.First()["x"], BsTypes.Create(memory, "int", 5));
            Assertions.AssertVariablesEqual(memory.Scopes.First()["y"], BsTypes.Create(memory, "int", 9));
        }
        ...
        public void ObjectContextBecomesFirstPositionalArgumentIfGiven()
        {
            ...
            {
                { "i", BsTypes.Create(memory, "int", 5) },
                { "j", BsTypes.Create(memory, "int", 6) }
            };
    ...
    public class WithVariableArguments
    {
        ...
        public void TreatsAllVariableArgumentsAsPositionalArguments()
        {
            // Function: (x, y)     Call: (5, 6)
            var memory = Runner.Run("");
            var arguments = new List<Variable>() { BsTypes.Create(memory, "int", 5), BsTypes.Create(memory, "int", 6) };
            var parameters = new List<Instruction>() { new VariableInstruction("x"), new VariableInstruction("y") };
            ArgumentTransfer.RunAndApply(memory, arguments, parameters);
            Assertions.AssertVariablesEqual(memory.Scopes.First()["x"], BsTypes.Create(memory, "int", 5));
            Assertions.AssertVariablesEqual(memory.Scopes.First()["y"], BsTypes.Create(memory, "int", 6));
        ...
        public void ObjectContextBecomesFirstPositionalArgumentIfGiven()
        {
            // Function: (x, y)     Call: (5)
            var memory = Runner.Run("");
            var arguments = new List<Variable>() { BsTypes.Create(memory, "int", 5) };
            var parameters = new List<Instruction>() { new VariableInstruction("x"), new VariableInstruction("y") };
            var classValues = new Dictionary<string, Variable>()
            {
                { "i", BsTypes.Create(memory, "int", 5) },
